  ...
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        ...
        expect(handle.message.error?.name).toBe("RoleMarkerHallucinationError")
        if (handle.message.error?.name !== "RoleMarkerHallucinationError") return
        expect(handle.message.error.data.code).toBe("ROLE_MARKER_HALLUCINATION")
      }),
    { config: (url) => providerCfg(url) },
  ),
)
